...
def violin_sentiment(conn, cursor):
    ...
    plt.title('Distribution of opening, closing and delta sentiment for British Airways (top-left), KLM (top right),'
              'RyanAir (bottom-left), and Lufthansa (bottom-right) ')
    ax = fig.add_subplot(gs[0, 3])
    sns.violinplot(data=df2['opening_tweet'], color='forestgreen', inner="quartile")
    ax.set_xlabel("opening_tweet")

    ax = fig.add_subplot(gs[0, 4])
    sns.violinplot(data=df2['last_tweet'], color='forestgreen', inner="quartile")
    ax.set_xlabel("last_tweet")

    ax = fig.add_subplot(gs[0, 5])
    sns.violinplot(data=df2['delta_sentiment'], color='forestgreen', inner="quartile")
    ax.set_xlabel("delta_sentiment")
    ...
    ax.set_xlabel("delta_sentiment")
    plt.title('Distribution of opening, closing and delta sentiment for RyanAir (left) and Lufthansa (right)')
...
def sentiment_year(conn, cursor):
    if 1:
        ...
        df1.groupby([df1['created_at'].dt.date])['sentiment'].mean().plot(kind='line')
        ...
        df1.groupby([df1['created_at'].dt.date])['sentiment'].count().plot.area(alpha=0.2, color=color)
    ...
    if 1:
        ...
        ax1.set_xlabel('Date')
        ax1.set_ylabel('Sentiment (0 is negative, 1 is positive)')
        df2.groupby([df2['created_at'].dt.date])['sentiment'].mean().plot(kind='line')
        plt.xticks(rotation=45)
        ...
        color = 'tab:red'
        ax2.set_ylabel('Number of tweets in the week', color=color)
        df2.groupby([df2['created_at'].dt.date])['sentiment'].count().plot.area(alpha=0.2, color=color)
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.xaxis.set_major_formatter(mdates.DateFormatter("%b-'%y"))
    ...
    if 1:
        ...
        ax1.set_xlabel('Date')
        ax1.set_ylabel('Sentiment (0 is negative, 1 is positive)')
        df3.groupby([df3['created_at'].dt.date])['sentiment'].mean().plot(kind='line')
        plt.xticks(rotation=45)
        ...
        color = 'tab:red'
        ax2.set_ylabel('Number of tweets in the week', color=color)
        df3.groupby([df3['created_at'].dt.date])['sentiment'].count().plot.area(alpha=0.2, color=color)
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.xaxis.set_major_formatter(mdates.DateFormatter("%b-'%y"))
    ...
    if 1:
        ...
        ax1.set_xlabel('Date')
        ax1.set_ylabel('Sentiment (0 is negative, 1 is positive)')
        df4.groupby([df4['created_at'].dt.date])['sentiment'].mean().plot(kind='line')
        plt.xticks(rotation=45)
        ...
        color = 'tab:red'
        ax2.set_ylabel('Number of tweets in the week', color=color)
        df4.groupby([df4['created_at'].dt.date])['sentiment'].count().plot.area(alpha=0.2, color=color)
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.xaxis.set_major_formatter(mdates.DateFormatter("%b-'%y"))
